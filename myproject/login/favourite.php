...
if (isset($_POST['delete'])){	
	...
	$sql = "delete from favourite WHERE item_id = '$delete_id' and user_id = '$user_id'";
	$dbcon->query($sql);
...
while ($item=$result->fetchArray(SQLITE3_ASSOC)){
    ...
    <a href=\"http://localhost/myproject/post.php?iid1=".$item["item_id"]."\"><center><img src=\"$img_src\" alt=\"".$item['item_name']."\" style=\"height:250px;\"></center>\
      <center><p>".$item["item_name"]."</p></a><form id=\"deleteForm\" method=\"post\" action=\"favourite.php\"><input name=\"delete\" value=\"".$item["item_id"]."\"hidden/><input type=\"submit\" class=\"btn btn-primary \" value=\"Delete\"/ style=\"background:red;margin-right:35%\"></form></center><br>\
  </div>')</script>";
}
if($uname != $sname){
